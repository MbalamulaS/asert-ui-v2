...
import { BedTypeFormComponent } from './forms/bed-type-form.component';
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Bed Types" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'BedType'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'BedType'"
            icon="launch"
          ...
          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'BedType'"
            icon="delete_outline"
            tooltip="Delete Bed Type"
            color="accent"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Bed Type"
      >
        <ng-template>
          <bed-type-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>
