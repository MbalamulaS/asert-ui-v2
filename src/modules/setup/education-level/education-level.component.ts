...
import {EducationLevelFormComponent} from "modules/setup/education-level/forms/education-level-form.component";
import {EducationLevel, EducationLevelService} from "modules/setup/education-level/education-level.service";
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Education Levels"/>
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'EducationLevel'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        ...
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'EducationLevel'"
            icon="launch"
            tooltip="Edit EducationLevel"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'EducationLevel'"
            icon="delete_outline"
            tooltip="Delete EducationLevel"
            color="accent"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Education Level"
      >
        <ng-template>
          <app-education-level-form
...
export class EducationLevelComponent implements OnInit {
  ...
  mapData = (data: EducationLevel[]) => {
    return data.map((item: EducationLevel) => ({
      ...item,
    }));
  };
}
