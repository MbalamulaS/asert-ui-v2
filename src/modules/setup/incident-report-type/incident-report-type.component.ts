...
import { FormComponent } from 'modules/setup/incident-report-type/forms/form.component';
import {  IncidentReportTypeService } from './incident-report-type.service';
import {  IncidentReportType } from './incident-report-type';

@Component({
  selector: 'app-incident-report-type',
  standalone: true,
  viewProviders: [
    provideIcons({
      heroCog6Tooth,
  ...
  template: `
    <container>
      <app-header title="Manage Incident Report Types" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant : 'create' : 'IncidentReportType'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant : 'update' : 'IncidentReportType'"
            icon="launch"
            tooltip="Edit Incident Report Type"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant : 'delete' : 'IncidentReportType'"
            icon="delete_outline"
            tooltip="Delete Incident Report Type"
            color="accent"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Incident Report Type"
      >
        <ng-template>
          <app-incident-report-type-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>

      <app-confirm-dialog
        [open]="isDialogOpen"
        [title]="'Delete Incident Report Type'"
        [message]="'Are you sure you want to delete this Incident Report Type?'"
...
export class IncidentReportTypeComponent implements OnInit {
  isOpen = false;
  isDialogOpen = false;
  listData: IncidentReportType[] = [];
  selectedItem!: IncidentReportType;
  title: string = '';
  ...
  constructor(
    public service: IncidentReportTypeService,
  ...
  async fetchData() {
    ...
    if (this.searchTerm && this.searchTerm.length > 0) {
      ...
      query['severity'] = this.searchTerm;
    }
