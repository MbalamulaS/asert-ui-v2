...
import {FacilityType, FacilityTypeService} from './facility-type.service';
import {FacilityTypeFormComponent} from './forms/facility-type-form.component';
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Facility Types"/>
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          icon="add"
          class="w-full md:w-auto"
          [isDisabled]="'' | cant: 'create' : 'FacilityType'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <div class="flex flex-row gap-1">
            <icon-button
              [isDisabled]="'' | cant: 'update' : 'FacilityType'"
              icon="launch"
              tooltip="Edit FacilityType"
              (action)="openDialog(item)"
              color="primary"
            />

            <icon-button
              [isDisabled]="'' | cant: 'delete' : 'FacilityType'"
              icon="delete_outline"
              tooltip="Delete FacilityType"
              color="accent"
      ...
      >
        <ng-template>
          <facility-type-form (onSubmit)="saveData($event)"/>
        </ng-template>
      </app-dialog>
...
export class FacilityTypeComponent implements OnInit {
  ...
  constructor(public service: FacilityTypeService) {
  }
