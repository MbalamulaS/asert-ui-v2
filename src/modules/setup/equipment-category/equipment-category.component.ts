...
} from './equipment-category.service';
import { EquipmentCategoryFormComponent } from './forms/equipment-category-form.component';
...
@Component({
  ...
  imports: [
    ...
    CantPipe,
    EquipmentCategoryFormComponent,
    WrapperComponent,
  ...
  template: `
    <container>
      <app-header title="Manage Equipment Categories" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'EquipmentCategory'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        ...
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'EquipmentCategory'"
            icon="launch"
            tooltip="Edit EquipmentCategory"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'EquipmentCategory'"
            icon="delete_outline"
            tooltip="Delete EquipmentCategory"
            color="accent"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} EquipmentCategory"
      >
        <ng-template>
          <equipment-category-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>

      <app-confirm-dialog
        [open]="isDialogOpen"
        [title]="'Delete EquipmentCategory'"
...
export class EquipmentCategoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.fetchEquipmentCategorys();
  }

  async fetchEquipmentCategorys() {
    let query = {
  ...
  navigateToEquipmentCategoryPermission(uuid: string): void {
  ...
  async saveData(data: any) {
    try {
      ...
      this.isOpen = false;
      await this.fetchEquipmentCategorys();
    } catch (error) {
      // Handle error
      console.log(error);
    }
  ...
  mapData = (data: EquipmentCategory[]) => {
    return data.map((item: EquipmentCategory) => ({
      ...item,
