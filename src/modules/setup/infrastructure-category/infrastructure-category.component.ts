...
import { FormComponent } from 'modules/setup/infrastructure-category/forms/form.component';
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Infrastructure Categories" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'InfrastructureCategory'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'InfrastructureCategory'"
            icon="launch"
          ...
          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'InfrastructureCategory'"
            icon="delete_outline"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Infrastructure Category"
      >
        <ng-template>
          <app-infrastructure-category-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>

      <app-confirm-dialog
        [open]="isDialogOpen"
        [title]="'Delete Infrastructure Category'"
        [message]="
          'Are you sure you want to delete this Infrastructure Category?'
        "
...
export class InfrastructureCategoryComponent implements OnInit {
  ...
  constructor(
    public service: InfrastructureCategoryService,
