...
export class InfrastructureCategoryService {
  private readonly API = 'infrastructure-categories';
  private items: InfrastructureCategory[] = [];
  ...
  populateForm(data: InfrastructureCategory) {
  ...
  create(role: InfrastructureCategory): Observable<ApiResponse> {
    return this.httpService.post<ApiResponse>(this.API, role);
  }

  update(uuid: string, item: InfrastructureCategory): Observable<ApiResponse> {
