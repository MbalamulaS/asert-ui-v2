...
import { FormComponent } from 'modules/setup/country/forms/form.component';
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Countries" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant : 'create' : 'Country'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant : 'update' : 'Country'"
            icon="launch"
          ...
          <icon-button
            [isDisabled]="'' | cant : 'delete' : 'Country'"
            icon="delete_outline"
      ...
      >
        <ng-template>
          <app-country-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>
...
export class CountryComponent implements OnInit {
  ...
  async fetchData() {
    ...
    if (this.searchTerm && this.searchTerm.length > 0) {
      ...
      query['countryCode'] = this.searchTerm;
      query['zipCode'] = this.searchTerm;
    }
