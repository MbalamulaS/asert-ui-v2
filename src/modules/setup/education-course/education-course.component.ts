...
import {EducationCourse, EducationCourseService,} from './education-course.service';
import {ContainerComponent} from 'components/container/container.component';
import {NgIf} from '@angular/common';
import {EducationCourseFormComponent} from "modules/setup/education-course/forms/education-course-form.component";
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Education Courses"/>
      <app-wrapper>
        <app-search
      ...
      >
        ...
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'EducationCourse'"
            icon="launch"
            tooltip="Edit Education Course"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'EducationCourse'"
            icon="delete_outline"
            tooltip="Delete Education Course"
            color="accent"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Education Course"
      >
        <ng-template>
          <app-education-course-form (onSubmit)="saveData($event)"/>
        </ng-template>
      </app-dialog>
...
export class EducationCourseComponent implements OnInit {
  ...
  columns = [
    ...
    {label: 'Level', value: 'educationLevelName'},
  ];
  ...
  mapData = (data: EducationCourse[]) => {
    return data.map((item: EducationCourse) => ({
      ...item,
    }));
  };
}
