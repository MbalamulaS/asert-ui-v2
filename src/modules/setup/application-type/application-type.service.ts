...
import {ApplicationType} from "modules/setup/application-type/application-type";

@Injectable({
  providedIn: 'root',
})
export class ApplicationTypeService {
  private readonly API = 'application-types'
  private items: ApplicationType[] = [];
  ...
  populateForm(data: ApplicationType) {
  ...
  create(role: ApplicationType): Observable<ApiResponse> {
    return this.httpService.post<ApiResponse>(this.API, role);
  }

  update(uuid: string, item: ApplicationType): Observable<ApiResponse> {
