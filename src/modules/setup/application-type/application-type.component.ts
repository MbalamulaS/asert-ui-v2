...
import { FormComponent } from 'modules/setup/application-type/forms/form.component';
import { ApplicationTypeService } from 'modules/setup/application-type/application-type.service';
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Application Types" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'ApplicationType'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'ApplicationType'"
            icon="launch"
            tooltip="Edit Application Type"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'ApplicationType'"
            icon="delete_outline"
            tooltip="Delete Application Type"
            color="accent"
      ...
      >
        <ng-template>
          <app-application-type-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>
...
export class ApplicationTypeComponent implements OnInit {
  ...
  constructor(
    public service: ApplicationTypeService,
