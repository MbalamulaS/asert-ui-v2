...
import { FacilityOperationFormComponent } from 'modules/setup/facility-operation/form/facility-operation-form.component';
...
@Component({
  selector: 'app-facility-operation',
  standalone: true,
  viewProviders: [
    provideIcons({
      heroCog6Tooth,
  ...
  imports: [
    ...
    CantPipe,
    FacilityOperationFormComponent,
    ContainerComponent,
    WrapperComponent,
  ...
  template: `
    <container>
      <app-header title="Manage Facility Operation Status" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'FacilityOperationStatus'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'FacilityOperationStatus'"
            icon="launch"
          ...
          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'FacilityOperationStatus'"
            icon="delete_outline"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Facility Operation Status"
      >
        <ng-template>
          <app-facility-operation-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>
...
export class FacilityOperationComponent implements OnInit {
  isOpen = false;
  isDialogOpen = false;
  data: FacilityOperationStatus[] = [];
  selectedIem!: FacilityOperationStatus;
  title: string = '';
  ...
  constructor(
    public service: FacilityOperationService,
  ...
  async saveData(data: FacilityOperationStatus) {
    try {
      if (data.uuid) {
