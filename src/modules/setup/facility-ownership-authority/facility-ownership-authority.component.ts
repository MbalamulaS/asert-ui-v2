...
import { FormComponent } from 'modules/setup/facility-ownership-authority/forms/form.component';

@Component({
  selector: 'app-facility-ownership-authority',
  standalone: true,
  viewProviders: [
    provideIcons({
      heroCog6Tooth,
  ...
  template: `
    <container>
      <app-header title="Manage Facility Ownership Authorities" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'FacilityOwnershipAuthority'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'FacilityOwnershipAuthority'"
            icon="launch"
            tooltip="Edit Infrastructure Authority"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'FacilityOwnershipAuthority'"
            icon="delete_outline"
      ...
      <app-dialog
        ...
        width="740px"
        title="{{ title }} Facility Ownership Authority"
      >
        <ng-template>
          <app-facility-ownership-authority-form
      ...
      <app-confirm-dialog
        [open]="isDialogOpen"
        [title]="'Delete Facility Ownership Authority'"
        [message]="
          'Are you sure you want to delete this Facility Ownership Authority?'
        "
...
export class FacilityOwnershipAuthorityComponent implements OnInit {
  isOpen = false;
  isDialogOpen = false;
  roles: FacilityOwnershipAuthority[] = [];
  selectedIem!: FacilityOwnershipAuthority;
  title: string = '';
  ...
  constructor(public service: FacilityOwnershipAuthorityService) {}
