...
import { PremiseService } from 'modules/setup/premise/premise.service';
import { FormComponent } from 'modules/setup/premise/forms/form.component';
...
@Component({
  ...
  template: `
    <container>
      <app-header title="Manage Premises" />
      <app-wrapper>
        <app-search
        ...
        <action-button
          ...
          class="w-full md:w-auto"
          icon="add"
          [isDisabled]="'' | cant: 'create' : 'Premise'"
          (action)="openDialog()"
        />
      </app-wrapper>
      <app-table
      ...
      >
        <ng-template #actionTemplate let-item>
          <icon-button
            [isDisabled]="'' | cant: 'update' : 'Premise'"
            icon="launch"
            tooltip="Edit Premise"
            (action)="openDialog(item)"
            color="primary"
          />

          <icon-button
            [isDisabled]="'' | cant: 'delete' : 'Premise'"
            icon="delete_outline"
      ...
      >
        <ng-template>
          <app-premise-form (onSubmit)="saveData($event)" />
        </ng-template>
      </app-dialog>
      ...
      <app-confirm-dialog
        ...
        [message]="'Are you sure you want to delete this Premise?'"
...
export class PremiseComponent implements OnInit {
  ...
  constructor(public service: PremiseService) {}
