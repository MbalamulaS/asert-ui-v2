...
@Component({
  selector: 'app-assessor-new-assignment',
  standalone: true,
  imports: [
    MatIconModule,
    MatButtonModule,
  ...
  template: `
    <container>
      <app-header title="New Assignments"/>
      <div
...
export class MyNewAssignmentComponent implements OnInit {
  ...
  async loadData() {
    ...
    this.isLoading = true
    this.service.myNewAssignments({...query, size: this.pageSize}).subscribe({
      next: (response) => {
