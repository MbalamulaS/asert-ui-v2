...
@Component({
  selector: 'app-assessor-completed-assignments',
  standalone: true,
  imports: [
    MatIconModule,
    MatButtonModule,
  ...
  template: `
    <container>
      <app-header title="Completed Assessments"/>
      <div
...
export class MyCompletedAssignmentComponent implements OnInit {
  ...
  loadData() {
    ...
    this.service.myCompletedAssignments({...query, size: this.pageSize}).subscribe({
      next: (response) => {
