...
@Component({
  ...
  template: `
    <container>
      <app-header title="Rejected Applications" />
...
export class AssessorRejectedApplicationComponent implements OnInit {
  isOpen = false;
  data: Assessor[] = [];
  ...
  columns = [
    { label: 'Date', value: 'dateRejected' },
    { label: 'First Name', value: 'firstName' },
    { label: 'Middle Name', value: 'middleName' },
    { label: 'Last Name', value: 'lastName' },
    { label: 'Rejection Reason', value: 'reasonDetail' },
  ];
  ...
  mapData = (data: Assessor[]) => {
    return data.map((item: Assessor) => ({
      ...item,
      reasonDetail: item.rejectionReason,
    }));
  };
