...
@Component({
  ...
  template: `
    ...
    >
      ...
      <span class="text-3xl font-medium text-red-500">Loading...</span>
    </div>
  `,
  animations: [
    trigger('fadeInOut', [
...
export class LoginPreloaderComponent implements OnInit, OnDestroy {
  loading = false;
  ...
  ngOnInit() {
    this.subscription = this.preloaderService.loading$.subscribe(
  ...
  getLoaderClasses(element: string): string {
    ...
    if (element === 'svg') {
      ...
      existingClasses = 'text-3xl font-medium text-red-500';
    }
